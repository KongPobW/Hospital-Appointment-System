...
public class Cancel extends JFrame {
	...
	public Cancel(JTextField username, JPasswordField password) {
		setIconImage(Toolkit.getDefaultToolkit().getImage("GUI/Favicon.png"));
		setTitle(MyApp.appName);
		...
		imgCan.setIcon(new ImageIcon("GUI/Cancel.jpg"));
		cancel.add(imgCan);
	}
	...
	private void cancelAppointment(JTextField username, JPasswordField password) {
		...
		String sqlSelect = "SELECT AppID, Appointment.UserName, PatPass FROM Patient INNER JOIN Appointment ON Patient.UserName = Appointment.UserName WHERE Appointment.AppID = " + appointID.getText() + " AND Patient.UserName = " + "'" + username.getText() + "' AND Patient.PatPass = " + "'" + pass_input + "'";
		...
		try {
			...
			if (rs.next()) {
				...
				String sqlDelete = "DELETE FROM Appointment WHERE AppID = " + appointID.getText() + " AND UserName = " + "'" + username.getText() + "'";
